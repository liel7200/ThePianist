...
public class Sheets extends AppCompatActivity implements View.OnTouchListener
{
    TextView puttext;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        puttext = (TextView) findViewById(R.id.puttext);
        creatSoundPool();
        loadSounds();
        matchToId();
        ...
        if (event.getAction() == MotionEvent.ACTION_UP)
        {
            for(int i=0;i<buttons.length;i++)
            {
                if(v == buttons[i])
                {
                    ...
                    puttext.setText(name[i]);
                    //puttext.setAlpha(0.0f);
                }
            }
            v.setAlpha(1);
        }
